...
struct RenderEngine
{
	...
	struct ReBuffer *(*CreateBuffer)(const struct ReBufferCreateInfo *bci);
	void (*UploadBuffer)(struct ReBuffer *buff, uint64_t offset, const void *data, uint64_t size);
	void (*DestroyBuffer)(struct ReBuffer *buff);

	struct ReTexture *(*CreateTexture)(const struct ReTextureCreateInfo *tci);
	void (*UploadTexture)(struct ReTexture *tex, const void *data, uint64_t size);
